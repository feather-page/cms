...
Given("the Unsplash API is available") do
  @unsplash_results = [
    {
      ...
      "urls" => { "thumb" => "https://images.unsplash.com/thumb.jpg", "regular" => "https://images.unsplash.com/regular.jpg" },
  ...
  stub_request(:get, /api\.unsplash\.com\/search\/photos/)
    .to_return(status: 200, body: { "results" => @unsplash_results }.to_json, headers: { 'Content-Type' => 'application/json' })
end
...
Given("an Unsplash photo {string} exists") do |photo_id|
  ...
  stub_request(:get, /api\.unsplash\.com\/photos\/#{photo_id}/)
  ...
  stub_request(:get, "https://images.unsplash.com/regular.jpg")
    .to_return(status: 200, body: File.read(Rails.root.join('spec/fixtures/files/15x15.jpg')), headers: { 'Content-Type' => 'image/jpeg' })
end
