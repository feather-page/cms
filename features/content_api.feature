...
Feature: Content API
  ...
  Scenario: Uploading an image file
    ...
    And the response should match the OpenAPI schema for "POST /sites/{site_id}/images" with status 201

  Scenario: Uploading a non-image file returns error
    When I upload a non-image file
    Then the response status should be 422

  Scenario: Creating an image from URL
    When I create an image from URL "https://example.com/photo.jpg"
    Then the response status should be 201
    And the response should contain an image ID
